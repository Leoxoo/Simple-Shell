...
void historyOffset(long offset) {
    ...
    if (offset >= 0 && offset < HISTORY_SIZE) { // Ensure offset is within valid range
        if (historyCount < HISTORY_SIZE) {
            ...
            /*
            long adjustedOffset;
            adjustedOffset = offset % HISTORY_SIZE;
            printf("This is the adjusted offset: %ld\n", adjustedOffset);
            */
            int adjustedOffset = (commandHistoryIndex + offset - 1) % HISTORY_SIZE;
            char* commandCopy = strdup(commandHistory[adjustedOffset]);
